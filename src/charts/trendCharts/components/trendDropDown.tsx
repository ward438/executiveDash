...
export const TrendDropDown = (
    ...
    { isOpen: boolean; setIsOpen: (isOpen: boolean) => void, selectedAccount: { account_name: string; cost: number; budget: number, date: string } | 
    null, setSelectedAccount: (selectedAccount: { account_name: string; cost: number; budget: number, date: string } | null) => void }
) => {     
    return <>  
        ...
        {isOpen && (
            <div className="absolute bg-white border z-10 flex flex-col w-80 max-h-64 overflow-y-auto shadow-md">
                ...
                {dateFilteredJson.filter((data) => data != null).map((data: any, index: number) => (
                    <button onClick={() => setSelectedAccount({account_name: data.account_name, cost: data.cost, budget: data.budget, date: data.date})} key={index} className="text-left px-3 py-1.5 text-sm hover:bg-yellow-100 border-b last:border-b-0">
